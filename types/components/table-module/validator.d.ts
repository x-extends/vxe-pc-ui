...
export interface TableValidatorMethods<D = any> {
   ...
   * 手动清除校验
   */
  clearValidate(rows?: any, fieldOrColumn?: VxeColumnPropTypes.Field | VxeTableDefines.ColumnInfo | VxeColumnPropTypes.Field[] | VxeTableDefines.ColumnInfo[]): Promise<void>
  /**
  ...
  /**
   * 快速校验，默认校验当前表格数据，如果存在记录不通过的记录，则返回不再继续校验（异步校验除外）；
  ...
  /**
   * 完整校验，默认校验当前表格数据，如果存在记录不通过的记录，则返回不再继续校验（异步校验除外）；
   * 如果第一个参数为 true 则校验当前表格数据，如果指定 row 或 rows 则校验指定一行或多行，如果不指定数据，则默认只校验临时变动的数据，例如新增或修改。返回一个 promise
   * @param rows 指定行
   * @param fieldOrColumn 指定字段
   */
  fullValidateField(rows?: boolean | object | any[], fieldOrColumn?: VxeColumnPropTypes.Field | VxeTableDefines.ColumnInfo | VxeColumnPropTypes.Field[] | VxeTableDefines.ColumnInfo[]): Promise<VxeTableDefines.ValidatorErrorMapParams<D>>
  /**
   * 快速校验，默认校验当前表格数据，如果存在记录不通过的记录，则返回不再继续校验（异步校验除外）；
   * 如果第一个参数为 true 则校验当前表格数据，如果指定 row 或 rows 则校验指定一行或多行，如果不指定数据，则默认只校验临时变动的数据，例如新增或修改。返回一个 promise
   * @param rows 指定行
   * @param fieldOrColumn 指定字段
   */
  validateField(rows?: boolean | object | any[], fieldOrColumn?: VxeColumnPropTypes.Field | VxeTableDefines.ColumnInfo | VxeColumnPropTypes.Field[] | VxeTableDefines.ColumnInfo[]): Promise<VxeTableDefines.ValidatorErrorMapParams<D>>
}

export interface TableValidatorPrivateMethods<D = any> {
  validCellRules(type: any, row: any, column: VxeTableDefines.ColumnInfo, val?: any): Promise<any>
  hasCellRules(type: any, row: any, column: VxeTableDefines.ColumnInfo): boolean
