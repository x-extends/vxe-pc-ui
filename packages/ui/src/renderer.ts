...
function getInputImmediateModel (renderOpts: any) {
...
function getCellEditProps (renderOpts: any, params: VxeGlobalRendererHandles.RenderEditParams, value: any, defaultProps?: any) {
...
function isImmediateCell (renderOpts: any, params: any) {
...
function getEditOns (renderOpts: any, params: any) {
  ...
  return getComponentOns(renderOpts, params, (cellValue: any) => {
    // 处理 model 值双向绑定
    if (isImmediate) {
      XEUtils.set(row, column.field, cellValue)
    } else {
      model.update = true
...
function nativeEditRender (renderOpts: any, params: any) {
  ...
  const { name } = renderOpts
  const cellValue = isImmediateCell(renderOpts, params) ? XEUtils.get(row, column.field) : column.model.value
  return [
    h(name, {
...
function getSelectCellValue (renderOpts: any, { row, column }: any) {
  ...
  if (!isEmptyValue(cellValue)) {
    return XEUtils.map(props.multiple
      ? cellValue
      : [cellValue]
    , optionGroups
      ? (value) => {
...
function handleExportSelectMethod (params: any) {
  ...
  return options.original ? XEUtils.get(row, column.field) : getSelectCellValue(column.editRender || column.cellRender, params)
}
...
const renderMap: Record<string, RendererOptions> = {
  ...
  VxeInput: {
    ...
    renderCell (renderOpts: any, params: any) {
      ...
      const { row, column } = params
      const digits = props.digits || globalConfigStore.input.digits
