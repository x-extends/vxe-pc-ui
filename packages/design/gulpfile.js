...
const exportModuleName = 'VxeDesign'
const esmOutDir = 'es'
...
gulp.task('build_umd', () => {
  return gulp.src('lib_temp/index.umd.js')
    .pipe(replace(`root["${exportModuleName}"] = factory(root["Vue"], root["VxeUI"], root["XEUtils"])`, `root["${exportModuleName}"] = factory(root["Vue"], root["VxeUI"], root["XEUtils"]).default`))
    .pipe(gulp.dest('lib'))
    .pipe(uglify())
    .pipe(rename({
      basename: 'index',
